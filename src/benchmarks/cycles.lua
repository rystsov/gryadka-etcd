...
function request()
    if state == 0 then
        local url = "http://127.0.0.1:2379/v2/keys/" .. key
        return wrk.format("GET", url, {}, nil)
    elseif state == 1 then
        local url     = "http://127.0.0.1:2379/v2/keys/" .. key .. "?prevExist=false"
        local body    = "value=0"
        local headers = {["Content-Type"] = "application/x-www-form-urlencoded"}
        return wrk.format("PUT", url, headers, body)
    elseif state == 2 then
        local url     = "http://127.0.0.1:2379/v2/keys/" .. key .. "?prevIndex=" .. ver
